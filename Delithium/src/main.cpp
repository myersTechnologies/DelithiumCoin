...
#if defined(NDEBUG)
# error "Delithiumcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1cffd9ee1ee78839d5ec1dee7567cfb5dbe1fb029c0e31da1ca53f8f70313b74");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Delithiumcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Delithiumcoin Signed Message:\n";
...
{
    // Delithiumcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 440000 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 440000); // Delithiumcoin: 440k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Delithiumcoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Delithiumcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Delithiumcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Delithiumcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcf0cd78a80b4589381794dfce8b6a33f1edd9a47bb3a56f55f0c1cd188ccc16e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "26/04/2022 Delithium apocalypse announced in star trek";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a27cd1421ea8fa7d860c3ff74331de7d55dbf94f8f0f276d179a3d39f384ba2946533795bf72a9605d5726940caf8eb40804d23ee9b418c84b435f8ed1a07c2b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1650965065;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085976803;

        if (fTestNet)
        {
            block.nTime    = 1650964949;
            block.nNonce   = 386396087;
        }
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc4ab67fbd667a4a7156d0fcd0b9998b438509fbb43291046c41f4db65d77a6ab"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xca, 0xe0, 0xaf }; // Delithiumcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DelithiumcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DelithiumcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DelithiumcoinMiner(CWallet *pwallet)
{
    printf("DelithiumcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("delithiumcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DelithiumcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DelithiumcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DelithiumcoinMiner, pwallet));
}
